...
void AssociationRules::FindPattern(OriginalGraph graph){
    ...
    //**Length 0 Frequent Paths i.e., Vertex attributes **//
    {
        ...
        if(ParallelFlag)ParallelFindFrequentSingleAttribute(graph);
        else FindFrequentSingleAttribute(graph);
        ...
        if(paths[0].size()==0)return;

        clock_gettime(CLOCK_REALTIME, &frequentattribute_startTime);
        cout << "---multiple attributes" << endl;

        vector<double> corescores(0,corenum);
//        corescores.resize(corenum);

        if(ParallelFlag&&PartitionMode!=RANDOM){
            cout<<"Reparitioning :" << PartitionMode<<endl;
            VerticesPartitioning(graph, PartitionMode);
        }

        if(EfficientFlag&&ParallelFlag)ParallelEfficientFindFrequentMultipleAttribute(graph);
        else if(EfficientFlag)EfficientFindFrequentMultipleAttribute(graph);
        else if(ParallelFlag)ParallelFindFrequentMultipleAttribute(graph);
        else FindFrequentMultipleAttribute(graph);
...
//        else FindFrequentMultipleAttribute(graph);
    ...
    //cout<<"frequent edge label size ="<<candidateedgelabels.size()<<endl;

//    int testcount=0;
//    for(auto& matchedvertex: matchedvertices){
//        if(Contains(matchedvertex.pathidentifiers[0],0)&&Contains(matchedvertex.targetidentifiers,0))testcount++;
//    }
//    cout<<"testcount: "<<testcount<<endl;
...

    {
        ...
        if(ParallelFlag&&EfficientFlag)ParallelEfficientFindFrequentLengthOnePath(graph);
        else if(EfficientFlag)EfficientFindFrequentLengthOnePath(graph);
        else if(ParallelFlag)ParallelFindFrequentLengthOnePath(graph,paths[0],graph.number_of_vertices);
        else FindFrequentLengthOnePath(graph,paths[0],graph.number_of_vertices);
        ...
        for (int for_length = 2; for_length < maximumpathlength + 1; for_length++) {
            if(ParallelFlag&&EfficientFlag)ParallelEfficientFindFrequentLengthLPath(graph, for_length);
            else if(EfficientFlag)EfficientFindFrequentLengthLPath(graph, for_length);
            else if(ParallelFlag)ParallelFindFrequentLengthLPath(graph, for_length,paths[for_length-1],graph.number_of_vertices);
    ...
    {
        if (EfficientFlag&&ParallelFlag)ParallelEfficientFindFrequentKleenePath(graph);
        else if (EfficientFlag)EfficientFindFrequentKleenePath(graph);
        else if (ParallelFlag)ParallelFindFrequentKleenePath(graph);
    ...
    if(ParallelFlag&&EfficientFlag)ParallelEfficientCombinePaths(graph);
    else if(EfficientFlag)EfficientCombinePaths(graph);
    else if(ParallelFlag)ParallelCombinePaths(graph);
...
void AssociationRules::FindFrequentSingleAttribute(OriginalGraph& graph){
    ...
    for(int for_cardinality = 0; for_cardinality < graph.cardinality_of_vertextuples; for_cardinality++){

        for(int i=0;i<graph.cardinality_of_edgelabels;i++){
            countinverseedgelabels[i]=0;
            //countindegrees_edgelabels[i]=0;
        }
        int count=0;
        ...
        for(auto vertex: graph.vertex_list){
            ...
            if(Contains(vertex.vertextuple,for_cardinality)){
                count++;
                ...
                for(int for_edgelabel=0;for_edgelabel<graph.cardinality_of_edgelabels;for_edgelabel++){
                    countinverseedgelabels[for_edgelabel]+=graph.vertex_inverseconnectingcount_edgelabels[vertex.vertexid][for_edgelabel];
                }
        ...
        if(count>minsupport){
            ...
            rulevertextuple.push_back(for_cardinality);
        ...
        for(int for_edgelabel=0;for_edgelabel<graph.cardinality_of_edgelabels;for_edgelabel++){
            ...
            for(int for_length=0;for_length<maximumpathlength;for_length++){
                ...
                if(countinverseedgelabels[for_edgelabel]!=0 && countinverseedgelabels[for_edgelabel]>minsupport/graph.maximumIndegree_edgelabel^(for_length)){
                    //cout<<globaltargetcount<<","<<for_edgelabel<<","<<for_length<<","<<countinverseedgelabels[for_edgelabel]<<endl;
                    vector<int> temptuples;
                    temptuples.push_back(for_cardinality);
                    ...
                    for(auto& matchedvertexid: matchedvertexids){
                        for(auto& edgeid : graph.vertex_connecting_edge_list[matchedvertexid]){
                            if(graph.edge_list[edgeid].edgelabel==for_edgelabel&&Contains(graph.vertex_list[graph.edge_list[edgeid].dst].vertextuple,for_cardinality)){
...
void AssociationRules::FindFrequentMultipleAttribute(OriginalGraph& graph){
...
void AssociationRules::EfficientFindFrequentMultipleAttribute(OriginalGraph& graph){
...
void AssociationRules::FindFrequentLengthOnePath(OriginalGraph &graph, vector<Path>& paths0set, int maxattributesize){
    ...
    for(Path path: paths0set){//extends paths, we call it path A
        ...
        for(auto matchedvertex: matchedvertices) {//for all vertices
            int pathid = Contains_ReturnIndex(matchedvertex.pathidentifiers[0], path.pathidentifier);//vertex matching the path A
            if (pathid > -1)matchedvertexs.push_back(matchedvertex.vertexid);
        }

        int numberOfmatchedvertex = matchedvertexs.size();
        ...
        for(auto for_edgelabel: candidateedgelabels[0]) {//for all edge labels
            ...
            for (auto& matchedvertex: matchedvertexs){

                for(int i=0;i<graph.number_of_vertices;i++)insertedtemptaregets[i]=false;
                temptargets.clear();

                for (auto edgeid: graph.vertex_connecting_edge_list[matchedvertex]) {// connecting edges to the end vertices

                    Edge tempedge = graph.edge_list[edgeid];
                    if (tempedge.edgelabel == for_edgelabel && !insertedtemptaregets[tempedge.dst]) {// the edges has the edge legbel
            ...
            for(auto path0: targettuples_edges[0][for_edgelabel]) {//for all frequent vertex tuples
                ...
                for (int for_vertex=0; for_vertex < matchedvertexs.size(); for_vertex++) {
                    for (auto target: targetslist[for_vertex]) {

                        if (Contains(matchedvertices[target].targetidentifiers,path0.first)) {

                            if (!insertedmatchedvertexs[matchedvertexs[for_vertex]]) {
                                matchedvertexids.push_back(matchedvertexs[for_vertex]);
                                count++;
                                insertedmatchedvertexs[matchedvertexs[for_vertex]] = true;
                            }
                            nexttargets[matchedvertexs[for_vertex]].push_back(target);
                        }
...
void AssociationRules::FindFrequentLengthLPath(OriginalGraph& graph, int length, int maxattributesize) {
    ...
    for(auto& path: paths[length-1]){//extends paths, we call it path A
        ...
        matchedvertex_pathidindexs.clear();

        for(auto matchedvertex: matchedvertices) {//for all vertices
            int pathid = Contains_ReturnIndex(matchedvertex.pathidentifiers[length - 1], path.pathidentifier);//vertex matching the path A
            if (pathid > -1)matchedvertex_pathidindexs.push_back(make_pair(matchedvertex.vertexid, pathid));
        }

        // cout<<"test L: matchedsize="<<matchedvertex_pathidindexs.size()<<endl;
...
//            int size=0;
//            for(int i=0;i<matchedvertex_pathidindexs.size();i++) {
//                    size += targetslist[i].size();
//            }
           // cout<<"test L: targetsize="<<size<<endl;
            ...
            for(auto path0: targettuples_edges[length-1][for_edgelabel]) {//for all frequent vertex tuples
                ...
                //cout<<"test L: "<<test<<"count="<<count<<endl;
...
void AssociationRules::EfficientFindFrequentKleenePath(OriginalGraph& graph) {
    ...
    for (auto for_edgelabel: candidateedgelabels[0]) {
        ...
        for (int for_vertex = 0; for_vertex < graph.number_of_vertices; for_vertex++) {
            ...
            if (matchedvertices[for_vertex].pathidentifiers[0].empty()||!graph.vertex_connectingcount_edgelabels[for_vertex][for_edgelabel]){
                edgetargetlists.push_back(temptargets);
                continue;
            }

            bfssearchvertex.push_back(for_vertex);

            for (int i = 0; i < graph.number_of_vertices; i++) {
                //insertednexttarget[i]=false;
                insertedtemptarget[i] = false;
            }
            insertedtemptarget[for_vertex] = true;
            ...
            for(int for_length=0;for_length<maximumpathlength;for_length++) {
                ...
                while (1) {
                    ...
                    int vertexid = bfssearchvertex.back();
                    bfssearchvertex.pop_back();

                    for (auto edgeid: graph.vertex_connecting_edge_list[vertexid]) {

                        Edge tempedge = graph.edge_list[edgeid];
                        if (tempedge.edgelabel == for_edgelabel) {

                            if (!insertedtemptarget[tempedge.dst]) {
                                if (tempedge.dst < for_vertex) {
                                    temptargets.push_back(tempedge.dst);
                                    insertedtemptarget[tempedge.dst] = true;
                                    for (auto target: edgetargetlists[tempedge.dst]) {
                                        if (!insertedtemptarget[target]) {
                                            temptargets.push_back(target);
                                            insertedtemptarget[target] = true;
                                        }
                                    }
                                } else {
            ...
            //clock_gettime(CLOCK_REALTIME, &frequentkleenepath_endTime);
            //cout<<"---DURING BFS ("<<for_vertex<<"/"<< graph.number_of_vertices<<", size="<<temptargets.size()<<"): ";
        ...
        int testcount=0;
        for (auto& sourcevertex: paths[0]) {// for the start of paths
            if(sourcevertex.vertextuples[0].size()>1)continue;
            ...
            for (auto targetvertex: targettuples_edges[1][for_edgelabel]) {// for the end of paths
                if(targetvertex.second.size() >1)continue;
                ...
                for (auto matchedvertexid: matchedvertexs_attributeset[sourcevertex.pathidentifier]) {

                    for (auto target: edgetargetlists[matchedvertexid]) {

                        if (Contains(matchedvertices[target].targetidentifiers,targetvertex.first)) {
                            ...
                            nexttargets[matchedvertexid].push_back(target);
                        }
                    }
                }
                ...
                if (count > minsupport) {
                    ...
                    globalpathidcount++;
                    temppath.count = count;
                    temppath.vertextuples.push_back(sourcevertex.vertextuples[0]);
                    temppath.vertextuples.push_back(targetvertex.second);
                    temppath.edgelabels.push_back(for_edgelabel);
                    ...
                    sourcevertex.verticalExtendingPathidentifiers.push_back(temppath.pathidentifier);
        ...
        for(auto& rulepath: candidatepaths){
            ...
            for (auto matchedvertex: matchedvertices) {
                ...
                for(auto& pathidentifier: rulepath.second){
                    int pathid = Contains_ReturnIndex(matchedvertex.pathidentifiers[1], pathidentifier);//vertex matching the path A
                    if (pathid == -1){
                        matchFlag=false;
                        break;
                    }
                    if(firstFlag){
                        nexttargets[matchedvertex.vertexid]=matchedvertex.targets[1][pathid];
                        firstFlag=false;
                    }
                    else{
                        nexttargets[matchedvertex.vertexid] = intersection(nexttargets[matchedvertex.vertexid], matchedvertex.targets[1][pathid]);

                        if(nexttargets[matchedvertex.vertexid].empty()){
                            matchFlag=false;
                            break;
                        }
                    }
                ...
                if(matchFlag){
                    count++;
                    matchedvertexids.push_back(matchedvertex.vertexid);
                }
            }
...
void AssociationRules::ParallelKleenePathMatching_perEdgeLabel(OriginalGraph& graph, int edgelabel){
    ...
    #pragma omp parallel for private(insertedtemptarget, temptargets, bfssearchvertex, bfssearchvertex_next, parallel_for_length, parallel_i, parallel_edgeid, tempedge, for_core_vertex, for_vertex, for_target,parallel_vertexid)
    for (int for_core = 0; for_core < corenum; for_core++) {
        for (for_core_vertex = 0; for_core_vertex < parallelvertexidsets[for_core].size(); for_core_vertex++) {

            for_vertex = parallelvertexidsets[for_core][for_core_vertex];
            //cout<<"bfs for_vertex1 : "<<for_vertex<<endl;
            temptargets.clear();
            bfssearchvertex.clear();

            // the vertex is unnecessary for check//
            if (matchedvertices[for_vertex].pathidentifiers[0].empty() || !graph.vertex_connectingcount_edgelabels[for_vertex][edgelabel]) {
                continue;
            }
            ...
            insertedtemptarget[for_vertex] = true;
            bfssearchvertex_next.clear();
            //cout<<"bfs for_vertex2 : "<<for_vertex<<endl;
            for (parallel_for_length = 0; parallel_for_length < maximumpathlength; parallel_for_length++) {
                ...
                while (1) {
                    ...
                    bfssearchvertex.pop_back();
                    //cout<<"test parallel "<<bfssearchvertex.size()<<","<<parallel_vertexid<<endl;
            ...
            edgetargetlists[for_vertex]=temptargets;
            bfsdoneflag[for_vertex]=true;
            //clock_gettime(CLOCK_REALTIME, &frequentkleenepath_endTime);
            //cout<<"---DURING BFS ("<<for_vertex<<"/"<< graph.number_of_vertices<<", size="<<temptargets.size()<<"): ";
            //cout<<(frequentkleenepath_endTime.tv_sec - frequentkleenepath_startTime.tv_sec) + (frequentkleenepath_endTime.tv_nsec - frequentkleenepath_startTime.tv_nsec) * pow(10, -9)<<endl;
        }
...
//    int bfscount=0;
//    for(int i=0;i<graph.number_of_vertices;i++){
//        bfscount+=edgetargetlists[i].size();
//    }
//    cout<<"test bfscount : " <<bfscount<<endl;
    //clock_gettime(CLOCK_REALTIME, &temp1_startTime);
...
void AssociationRules::FindFrequentKleenePath(OriginalGraph& graph) {
...
void AssociationRules::EfficientFindFrequentLengthOnePath(OriginalGraph &graph){



    vector<vector<Path>> paths_0;
    ...
    for(int i=0;i<paths[0].size();i++){
        paths_0[paths[0][i].vertextuples[0].size()-1].push_back(paths[0][i]);
    }


    FindFrequentLengthOnePath(graph,paths_0[0],1);
...
void AssociationRules::EfficientFindFrequentLengthLPath(OriginalGraph &graph, int length){
    ...
    FindFrequentLengthLPath(graph,length,1);
    ...
    while(1) {
        ...
        for (int for_path1 = start; for_path1 < end - 1; for_path1++) {
            ...
            for (int for_path2 = for_path1 + 1; for_path2 < end; for_path2++) {
                Path candidatepath2;
                vector<int> pathidentifiers2;
                pathidentifiers2.clear();
        ...
        for(auto& rulepath: candidatepaths){
            ...
            for (auto matchedvertex: matchedvertices) {
                ...
                for(auto& pathidentifier: rulepath.second){
                    int pathid = Contains_ReturnIndex(matchedvertex.pathidentifiers[length], pathidentifier);//vertex matching the path A
                    if (pathid == -1){
                        matchFlag=false;
                        break;
                    }
                    if(firstFlag){
                        nexttargets[matchedvertex.vertexid]=matchedvertex.targets[length][pathid];
                        firstFlag=false;
                    }
                    else{
                        nexttargets[matchedvertex.vertexid] = intersection(nexttargets[matchedvertex.vertexid], matchedvertex.targets[length][pathid]);
                        if(nexttargets[matchedvertex.vertexid].empty()){
                            matchFlag=false;
                            break;
                        }
...
//                cout<<"Start"<<endl;
//                for(int for_core=0;for_core<corenum;for_core++) {
//                    for (int for_vertex=0; for_vertex < matchedvertexsets[for_core].size(); for_vertex++) {
//
//                        cout<<matchedvertexsets[for_core][for_vertex].first<<": ";
//                        for(int i=0;i<targetslist[for_core][for_vertex].size();i++){
//                            cout<<" "<<targetslist[for_core][for_vertex][i];
//                            if(Contains(matchedvertices[targetslist[for_core][for_vertex][i]].targetidentifiers,0)){
//                                cout<<"(T)";
//                            }
//                            else cout<<"(F)";
//                        }
...
//        for(int for_core=0;for_core<corenum;for_core++) {
//            for (int i = 0; i < graph.cardinality_of_edgelabels; i++) {
//                parallelcountinverseedgelabels[for_core][i] = 0;
//            }
//        }
//
//        #pragma omp parallel for
//        for(int for_core=0;for_core<corenum;for_core++) {
//            matchedvertexids[for_core].clear();
//            //matchedvertexids[for_core]=
//            ParallelVertexAttributeMatching(graph, matchedvertexids[for_core], parallelvertexidsets[for_core], for_attribute, parallelcountinverseedgelabels[for_core]);
//        }
...
//            count+=matchedvertexids[for_core].size();
//            for(int i=0;i<graph.cardinality_of_edgelabels;i++){
//                countinverseedgelabels[i]+=parallelcountinverseedgelabels[for_core][i];
//            }
//        }

        //if(count!=graph.numbers_of_attributes[for_attribute])cout<<count<<","<<graph.numbers_of_attributes[for_attribute]<<endl;
...
//        for(int for_core=0;for_core<corenum;for_core++) {
//            for (int i = 0; i < graph.cardinality_of_edgelabels; i++) {
//                parallelcountedgelabels[for_core][i] = 0;
//            }
//        }
        #pragma omp parallel for
        for (int i = 0; i < graph.cardinality_of_edgelabels; i++) {
...
//        vector<int> tempvertexids;
//        tempvertexids.clear();
//        for (int for_core = 0; for_core < corenum; for_core++) {
//            for(auto& vertexid: matchedvertexids[for_core])tempvertexids.push_back(vertexid);
//        }
//
...
void AssociationRules::CombinePaths(OriginalGraph graph){
        ...
        for(int for_length2=for_length1;for_length2<maximumpathlength+1;for_length2++){
            ...
            for(int for_path1 = 0; for_path1 < paths[for_length1].size();for_path1++){
                ...
                for(int for_path2 = start; for_path2 < paths[for_length2].size();for_path2++){
                     ...
                     //pair<int,int> pathpair1 = pathidentifier2pathsindex[path1.pathidentifier];
                     //pair<int,int> pathpair2 = pathidentifier2pathsindex[path2.pathidentifier];

                     //cout<<pathid1<<"("<<pathpair1.first<<","<<pathpair1.second<<")"<<","<<pathid2<<"("<<pathpair2.first<<","<<pathpair2.second<<")"<<endl;
                     ...
                     for(auto matchedvertex: matchedvertices){
                         ...
                         int pathid1_index=Contains_ReturnIndex(matchedvertex.pathidentifiers[for_length1], pathid1);
                         int pathid2_index=Contains_ReturnIndex(matchedvertex.pathidentifiers[for_length2], pathid2);

                         if(pathid1_index>-1 && pathid2_index>-1){
                             count_s++;
                             matchedvertexids_s.push_back(matchedvertex.vertexid);

//                             if(Overlap(matchedvertex.targets[for_length1][pathid1_index],matchedvertex.targets[for_length2][pathid2_index])){
//                                 count_st++;
//                                 matchedvertexids_st.push_back(matchedvertex.vertexid);
//                             }
                         }
                     }
...
//
//                             temppattern.vertextuples.push_back(path2.vertextuples[path2.vertextuples.size()-1]);
//                             temppatternpath.source=0;
//                             temppatternpath.target=2;
...
void AssociationRules::EfficientCombinePaths(OriginalGraph graph){

    int combinecount=0;

    for(int for_path1 = 0; for_path1 < paths[1].size();for_path1++){

         for(int for_path2 = for_path1+1; for_path2 < paths[1].size();for_path2++){

             //cout<<for_path1<<","<<for_path2<<endl;

             Path path1 =  paths[1][for_path1];
             Path path2 =  paths[1][for_path2];
             int pathid1 = path1.pathidentifier;
             int pathid2 = path2.pathidentifier;
            ...
            combinecount++;
             for(auto matchedvertex: matchedvertices){
                 ...
                 int pathid1_index=Contains_ReturnIndex(matchedvertex.pathidentifiers[1], pathid1);
                 int pathid2_index=Contains_ReturnIndex(matchedvertex.pathidentifiers[1], pathid2);

                 if(pathid1_index>0 && pathid2_index>0){
                     count_s++;
                     matchedvertexids_s.push_back(matchedvertex.vertexid);
    ...
    while(1){
        ...
        for(int for_pattern=start;for_pattern<end;for_pattern++) {
            ...
            Pattern pathpattern = patterns[start];
            ...
            vector<int> frequentextendingids1;
            vector<int> frequentextendingids2;
            ...
            for (auto &extendingidentifiers: path1.verticalExtendingPathidentifiers) {
                ...
                for (auto matchedvertex: matchedvertices) {
                    ...
                    int pathid1_index = Contains_ReturnIndex(matchedvertex.pathidentifiers[pathindex1.first+1], extendingidentifiers);
                    int pathid2_index = Contains_ReturnIndex(matchedvertex.pathidentifiers[pathindex2.first], pathid2);
                ...
                if (count_s > minsupport) {
                    frequentextendingids1.push_back(extendingidentifiers);

                    Pattern temppattern;
                    temppattern.patternidentifier = patterns.size();
                    temppattern.count = count_s;

                    temppattern.pathidentifiers.push_back(extendingidentifiers);
                    temppattern.pathidentifiers.push_back(pathid2);
            ...
            for (auto &extendingidentifiers: path2.verticalExtendingPathidentifiers) {
                ...
                for (auto matchedvertex: matchedvertices) {
                    ...
                    int pathid1_index = Contains_ReturnIndex(matchedvertex.pathidentifiers[pathindex1.first], pathid1);
                    int pathid2_index = Contains_ReturnIndex(matchedvertex.pathidentifiers[pathindex2.first+1], extendingidentifiers);
                ...
                if (count_s > minsupport) {
                    frequentextendingids2.push_back(extendingidentifiers);
            ...
            while(1){
                ...
                if(frequentextendingids1[pos1]==frequentextendingids2[pos2]){
                    frequentextendingids2.erase(frequentextendingids2.begin()+pos2);
                }
                else if(frequentextendingids1[pos1]<frequentextendingids2[pos2]){
                    pos1++;
                }
                else{
                    pos2++;
            ...
            for (auto &extendingidentifiers1: frequentextendingids1) {
                for (auto &extendingidentifiers2: frequentextendingids2) {

                    if(extendingidentifiers1==extendingidentifiers2)continue;

                    pair<int, int> pathindex1 = pathidentifier2pathsindex[extendingidentifiers1];
                    pair<int, int> pathindex2 = pathidentifier2pathsindex[extendingidentifiers2];
                    ...
                    for (auto matchedvertex: matchedvertices) {
                        ...
                        int pathid1_index = Contains_ReturnIndex(matchedvertex.pathidentifiers[pathindex1.first+1], extendingidentifiers1);
                        int pathid2_index = Contains_ReturnIndex(matchedvertex.pathidentifiers[pathindex2.first+1], extendingidentifiers2);
                    ...
                    if (count_s > minsupport) {
                        ...
                        temppattern.pathidentifiers.push_back(extendingidentifiers1);
                        temppattern.pathidentifiers.push_back(extendingidentifiers2);
    ...
    for(int for_length=1;for_length<maximumpathlength+1;for_length++) {
        ...
        for (int for_path1 = 0; for_path1 < paths[for_length].size(); for_path1++) {
            ...
            for (int for_path2 = for_path1 + 1; for_path2 < paths[for_length].size(); for_path2++) {
                ...
                //cout <<"test2: "<< for_path1 << "," << for_path2 << endl;
...
//
    ...
    cout<<"unit path combination done"<<endl;
    int start=0;
    int end=patterns.size();
    while(1){
...
//                if(path1.pathidentifier==175||path1.pathidentifier==106){
...
//                }
                ...
                if(CheckPathDominance(expath,path2))continue;

                string temppair=to_string(extendingidentifiers)+"-"+to_string(pathid2);
                if(expath>path2){
                    string temppair=to_string(pathid2)+"-"+to_string(extendingidentifiers);
                }
...
//                if(path2.pathidentifier==69||path2.pathidentifier==106){
...
//                }
                ...
                if(CheckPathDominance(path1,expath))continue;

                string temppair=to_string(extendingidentifiers)+"-"+to_string(pathid1);
                if(expath>path1){
                    string temppair=to_string(pathid1)+"-"+to_string(extendingidentifiers);
                }
                if(findPatterns.find(temppair)!=findPatterns.end())continue;
                findPatterns[temppair]=true;
...
//                if(path1.pathidentifier==69||path1.pathidentifier==106){
...
//                int temppathid1=extendingidentifiers;
//                int temppathid2=pathid1;
//                if(expath>path1){
//                    temppathid1=pathid1;
//                    temppathid2=extendingidentifiers;
//                }
//                if(findPatterns.find(make_pair(temppathid1,temppathid2))!=findPatterns.end())continue;
//                findPatterns[make_pair(temppathid1,temppathid2)]=true;
...
void AssociationRules::ShowAllPaths(string dataname){
...
void AssociationRules::ShowAllPatterns(string dataname){
