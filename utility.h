...
//    int maintenanceWorkloadNum;
    ...
    void Output() {
        ...
        fout << inputFile<<"\t" << efficientFlag <<"\t" << coreNum<< "\t" << minsup<<"\t"<<maxlength<< "\t"<<partitionmode<< "\t" << attributeTime <<"\t"<<lengthoneTime <<"\t"<<lengthLTime <<"\t"<<kleeneTime <<"\t"<<combineTime   << "\t" << miningTime<< "\t" << pathNum<< "\t" << patternNum<<  std::endl;
    }
